    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {highRiskList.map((m) => {
          ...
          return (
            <div key={m.mineId} className="mg-card border-l-4 border-l-mgRed-500 p-5 shadow-card space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-enterprise-border">
                <div>
                  <h3 className="text-base font-bold text-enterprise-text flex items-center gap-2">
                    {m.mineName}
                ...
                <div className="text-right font-mono">
                  <span className="text-2xl font-extrabold text-mgRed-600">{m.complianceScore}%</span>
                  <p className="text-[10px] text-enterprise-text-muted">Compliance Score</p>
                </div>
              </div>

              <div className="text-xs space-y-2">
                <p className="font-semibold text-enterprise-text">Active High-Consequence Violations ({mineViolations.length}):</p>
                {mineViolations.map(v => (
                  <div key={v.violationId} className="p-2.5 bg-gray-50 rounded-lg border border-enterprise-border text-[11px] flex justify-between items-center">
                    <div>
                      <span className="font-mono font-bold text-mgRed-600">{v.violationId}</span>: {v.description}
                    </div>
              ...
              <div className="pt-2 border-t border-enterprise-border flex justify-end">
                <button
                  onClick={() => onSelectMine && onSelectMine(m)}
                  className="px-4 py-1.5 bg-mgRed-600 hover:bg-mgRed-500 text-white font-bold text-xs rounded-lg shadow-sm flex items-center gap-1.5 transition-colors"
                >
